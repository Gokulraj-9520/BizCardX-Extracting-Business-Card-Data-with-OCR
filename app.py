...
st.markdown("<h3 style='text-align: center; color: white; '><i>BizCardX: Extracting Business Card Data with OCR</i></h3>", unsafe_allow_html=True)

# SETTING-UP BACKGROUND IMAGE   #4e4376    #92CED3
def setting_background():
    st.markdown(f""" 
    <style>
        .stApp {{
            background: linear-gradient(to right, #92CED3, #43dad3);
        ...
        def get_data(res):
            ...
            for ind,i in enumerate(res):
                ...
                # To get EMAIL ID
                    ...
                    data["email"].append(i.strip())

                # To get MOBILE NUMBER
                elif "-" in i:
                    data["mobile_number"].append(i.strip())
                ...
                elif all(word.isalpha() for word in i.split()):
                    #print(i)
                    data['company_name'].append(i)
                    if len(data['company_name'])>1:
                        data['company_name']=" ".join(data['company_name'])
                ...
                if len(i)>=6 and i.isdigit():
                    data["pin_code"].append(i.strip())
                elif re.findall('[a-zA-Z]{9} +[0-9]',i):
                    data["pin_code"].append(i[10:].strip())
            #rint(data['area'])
            #print(data['city'])
            #print(data['state'])
            #print(data['card_holder'])
